...
/**
 * Ensures the repo is owned by the polarityd user
 *
 * @returns {Promise<string | Buffer>}
 */
async function setFilePermissions(){
  const { stdout, stderr }  = await exec('chown -R polarityd:polarityd ./misp-warninglists');
  if (stderr) {
    throw new Error(stderr);
  } else {
    return stdout;
...
async function run(doRemote) {
  try {
    if (!isCloned() && doRemote) {
    ...
    if (doRemote) {
      ...
      let stdout = await updateRepo();
      console.info(stdout);
      stdout = await setFilePermissions();
      console.info(stdout);
    }
